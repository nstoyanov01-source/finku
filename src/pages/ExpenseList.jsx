...
export default function ExpenseList({ session, language }) {
      ...
      <div className="list-page">
        ...
        <div className="list-content">
          ...
          {loading ? (
            ...
            filtered.map(row => (
              <div key={row.id} className="list-entry-row" onClick={() => setDrawer({ entry: row, type: 'expense' })}>
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div className="list-entry-desc">{row.description}</div>
                  <div className="list-entry-sub">
                    {lang.categories?.[row.category] ? `${lang.categories[row.category]} · ` : ''}{formatDate(row.date, language)}
                  </div>
                </div>
                <div style={{ fontSize: 14, fontWeight: 500, color: '#e07070', flexShrink: 0, marginLeft: 16, fontVariantNumeric: 'tabular-nums' }}>
